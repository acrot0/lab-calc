...
pH 标签页给出同样的两个数并排：理想模型与活度校正后。校正需要两个 pKa 不携带的输入——酸型电荷 z 与背景离子强度 I，两者默认 0（此时校正归零，返回理想答案）。**带电酸才会移动**，I = 0.1 实测：磷酸 pKa₂（z = −1）4.100 → 3.993，铵（z = +1）5.125 → 5.232，乙酸（z = 0）2.880 → 2.884。校正量在 I ≈ 0.5 达峰后回落，超过该点界面标注不可信。
...
- 溶液配制、稀释等按配制量回答的标签页仍按理想溶液处理——活度不影响称多少克
- 滴定曲线在等当点附近为近似值
...
| **pH 计算** | 弱酸/弱碱溶液的 pH（含常见缓冲体系 pKa 预设，带酸型电荷）；理想值与活度校正值并排显示；可展开各质子化态的分布曲线 |
...
| **反应计量** | 配平化学方程式（精确有理数零空间，非最小二乘拟合）、限量试剂、产率、经验式；可展开产量随投料量的变化曲线 |
| **电化学** | Nernst 方程、电池电动势、平衡常数；可展开 E–lg Q 直线，看工作点离平衡还有多远 |
